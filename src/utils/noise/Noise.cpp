...
// [0,1]
float Noise::noise2d(int x, int y, int seed)
{
...
float Noise::noise2dGradient(float x, float y, int seed)
{
...
// [0,1]
float Noise::noise3d(int x, int y, int z, int seed)
{
    // "hazard" from known random sequences, seed and coordinates
...
float Noise::noise3dGradient(float x, float y, float z, int seed)
{
    // Calculate the integer coordinates
    int x0 = (x > 0.0 ? (int)x : (int)x - 1);
    int y0 = (y > 0.0 ? (int)y : (int)y - 1);
